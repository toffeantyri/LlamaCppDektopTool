    ...
    for (int64_t ngram_size = ngram_min; ngram_size <= ngram_max; ++ngram_size) {
        ...
        for (int64_t i = i_start; i < inp_size; ++i) {
            ...
            if (print_progress && n_done % 10000000 == 0) {
                ...
                const int64_t eta_ms = (inp_size * (ngram_max - ngram_min + 1) - n_done) *
                                       (t_now_ms - t_start_ms) / n_done;
                const int64_t eta_min = eta_ms / (60 * 1000);
                const int64_t eta_s = (eta_ms - 60 * 1000 * eta_min) / 1000;

                fprintf(stderr,
                        "%s: %" PRId64 "/%" PRId64 " done, ETA: %02" PRId64 ":%02" PRId64 "\n",
...
static llama_token
get_token(const std::vector<llama_token> &inp, const std::vector<llama_token> &draft,
          const size_t i) {
...
static llama_token try_draft(
        common_ngram_cache &nc_primary, const std::vector<common_ngram> &ngrams_primary,
    ...
    for (int i = ngrams_primary.size() - 1; i >= 0 && drafted_token == LLAMA_TOKEN_NULL; --i) {
        ...
        for (std::pair<llama_token, int> token_count_primary: part_primary) {
            ...
            const int32_t count_static =
                    token_count_static_it != part_static.end() ? 100 * token_count_static_it->second
                                                               : 1;
...
void common_ngram_cache_draft(
    ...
    while ((int) draft.size() - 1 < n_draft) {
        ...
        if (drafted_token == LLAMA_TOKEN_NULL) {
            drafted_token = try_draft(nc_context, ngrams_cd, part_static, draft_min_sample_size_lax,
                                      draft_min_percent_lax);
        }
        if (drafted_token == LLAMA_TOKEN_NULL) {
            drafted_token = try_draft(nc_dynamic, ngrams_cd, part_static,
...
void common_ngram_cache_merge(common_ngram_cache &ngram_cache_target,
    ...
    for (std::pair<common_ngram, common_ngram_cache_part> ngram_part: ngram_cache_add) {
        ...
        for (std::pair<llama_token, int32_t> token_count: part) {
            ...
            common_ngram_cache_part::iterator token_count_merged_it = part_merged_it->second.find(
                    token);
