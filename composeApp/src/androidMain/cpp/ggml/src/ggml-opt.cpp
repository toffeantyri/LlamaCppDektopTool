...
static void ggml_opt_build(ggml_opt_context_t opt_ctx) {
    ...
    if (!opt_ctx->ctx_static) {
        ...
        const size_t tensors_per_param = (accumulate ? 1 : 0) +
                                         (opt_ctx->build_type_alloc == GGML_OPT_BUILD_TYPE_OPT ? 2
                                                                                               : 0);
    ...
    for (int i = opt_ctx->gf->n_nodes - 1; i >= 0; --i) {
        ...
        if (grad && (node->flags & GGML_TENSOR_FLAG_PARAM)) {
            ...
            struct ggml_tensor *opt_step = ggml_opt_step_adamw(opt_ctx->ctx_compute, node, grad, m,
                                                               v, opt_ctx->adamw_params);
            ...
            ggml_set_name(opt_step,
                          (std::string("AdamW step for ") + std::string(node->name)).c_str());
